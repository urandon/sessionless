...
// Package tests may enable the exact profile to prove its native protocol.
...
func (driver *Driver) Execute(ctx context.Context, request ports.ExecutionRequest, sink ports.ExecutionEventSink) (ports.ExecutionResult, error) {
	...
	invocation := ProcessInvocationV1{
		...
		ConfigDirectoryEnvironmentName: ConfigEnvironmentV1, ConfigDirectoryMountPath: ConfigMountPathV1,
		RequirePrivateWorkingDirectory: true, RequireSanitizedEnvironment: true, RequireNoAmbientHome: true,
	...
	parsed := codexexec.ParseProtocolV1(process.Stdout)
	zero(process.Stdout)
	defer zero(parsed.Final)
	...
	if err := sink.Emit(ctx, ports.ExecutionEvent{Sequence: 1, Boundary: "codex.turn_completed"}); err != nil {
		result.Summary = ""
		return result, backendError(domain.ErrorTerminal)
	}
	return result, nil
...
func reduceEvidence(binding domain.HarnessBindingV1, parsed codexexec.ProtocolResultV1, process ProcessResultV1, runErr error) (domain.ProviderExecutionEvidenceV1, error) {
	...
	case parsed.Terminal && runErr == nil && process.ExitCode == 0 && process.StdoutBytes == len(process.Stdout) &&
		process.StdoutBytes <= maxOutputBytes && process.StderrBytes >= 0 && process.StderrBytes <= maxStderrBytes &&
		!process.OutputLimitExceeded && process.ProcessStopped && process.DescendantsStopped && process.CleanupSucceeded &&
		process.PrivateStateRemoved && process.CredentialFinalized && process.ProviderEffectFenceSatisfied &&
		process.ProviderEffects == maxProviderEffects && process.FailureCode == "":
...
func compilePrompt(request ports.ExecutionRequest) ([]byte, error) {
	...
	if records[len(records)-1].Event.Sequence != request.ContextWindow.ThroughSequence {
		return nil, ErrContract
	}
...
func backendError(kind domain.ErrorKind) error {
	return &domain.ClassifiedError{Kind: kind, Code: string(sessionlessharness.FailureHarnessBackendFailed), Operation: "codex_openrouter.adapter"}
}
