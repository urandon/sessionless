...
func (driver *Driver) Execute(ctx context.Context, request ports.ExecutionRequest, sink ports.ExecutionEventSink) (ports.ExecutionResult, error) {
	...
	requestID := "sessionless-" + string(request.AttemptID)
	command, err := json.Marshal(struct {
		ID      string `json:"id"`
		Type    string `json:"type"`
		Message string `json:"message"`
	}{ID: requestID, Type: "prompt", Message: string(prompt)})
	zero(prompt)
	if err != nil || len(command)+1 > maxPromptBytes {
		zero(command)
		return ports.ExecutionResult{}, backendError(domain.ErrorTerminal)
	}
	...
	if err := sink.Emit(ctx, ports.ExecutionEvent{
		Sequence: 1, Boundary: "pi.agent_settled", InputTokens: cloneUint64(parsed.inputTokens), OutputTokens: cloneUint64(parsed.outputTokens),
	}); err != nil {
		result.Summary = ""
		return result, backendError(domain.ErrorTerminal)
	}
...
func reduceEvidence(binding domain.HarnessBindingV1, parsed rpcResult, process ProcessResultV1, runErr error) (domain.ProviderExecutionEvidenceV1, error) {
	...
	case parsed.terminal && runErr == nil && process.ExitCode == 0 && process.StdoutBytes == len(process.Stdout) &&
		...
		process.ProcessStopped && process.CleanupSucceeded && process.CredentialFinalized && process.FailureCode == "":
	...
	case !process.ProcessStopped || !process.CleanupSucceeded:
		evidence.FinishClass = domain.ProviderFinishFailedV1
...
func backendError(kind domain.ErrorKind) error {
	return &domain.ClassifiedError{Kind: kind, Code: string(sessionlessharness.FailureHarnessBackendFailed), Operation: "pi_openrouter.adapter"}
}
