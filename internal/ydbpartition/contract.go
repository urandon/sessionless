...
var policies = []Policy{
	...
	hot("provider_credential_bindings", []string{"tenant_id", "owner_user_id", "resource_kind", "resource_id"}, ClassEntity, "provider API and router credentials are exact owner-scoped resource authorities"),
	hot("provider_credential_audit_events", []string{"tenant_id", "owner_user_id", "resource_kind", "resource_id", "resource_revision"}, ClassOrdered, "content-free credential mutation receipts are ordered behind one owner-scoped resource"),
	hot("provider_credential_candidate_fences", []string{"tenant_id", "owner_user_id", "resource_kind", "resource_id", "mutation_id"}, ClassOrdered, "candidate cleanup tombstones serialize secret deletion against a later binding CAS"),
	hot("provider_credential_cleanups", []string{"tenant_id", "owner_user_id", "resource_kind", "resource_id", "credential_generation", "secret_ref"}, ClassOrdered, "superseded opaque secret locators are bounded behind one owner-scoped provider resource"),
	{
		LogicalName: "provider_credential_cleanup_ready", PhysicalTable: "provider_credential_cleanup_ready_v1",
		PrimaryKey: []string{"shard_bucket", "created_at", "tenant_id", "owner_user_id", "resource_kind", "resource_id", "credential_generation", "secret_ref"},
		Class:      ClassGlobalReady, LoadPartitioning: true, Bucketed: true,
		Rationale: "bounded 16-way recovery of superseded provider secret locators without a new owner mutation",
	},
	...
	entity("managed_execution_authority_v2_cutover_state", []string{"cutover_id"}, "one explicit empty-backlog placement+harness+substrate+cost cutover marker"),
	hot("attempt_effect_reservations", []string{"tenant_id", "attempt_id", "kind"}, ClassEntity, "one immutable provider-effect fence per attempt and effect kind"),
