...
if (!/^v(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)(?:-rc\.(?:0|[1-9][0-9]*))?$/.test(tag)) {
  throw new Error("RELEASE_TAG is not an accepted Sessionless release tag");
}
...
let oidcToken;
if (process.env.RELEASE_OIDC_TEST_TOKEN && process.env.GITHUB_ACTIONS !== "true") {
  oidcToken = process.env.RELEASE_OIDC_TEST_TOKEN;
} else {
...
if (claims.ref !== expectedRef || required("GITHUB_REF") !== expectedRef) {
  throw new Error("release identity is restricted to the selected tag ref");
}
if (claims.sha !== sourceSHA || required("GITHUB_SHA") !== sourceSHA) {
  throw new Error("release identity source differs from the peeled tag commit");
}
if (!['push', 'workflow_dispatch'].includes(claims.event_name) ||
    claims.event_name !== required("GITHUB_EVENT_NAME")) {
  throw new Error("unexpected GitHub OIDC event");
}
...
if (claims.sub !== required("YANDEX_EXPECTED_OIDC_SUBJECT")) {
  throw new Error("GitHub OIDC subject differs from the Terraform-bound release subject");
}

const exchangeBody = new URLSearchParams({
  grant_type: "urn:ietf:params:oauth:grant-type:token-exchange",
  requested_token_type: "urn:ietf:params:oauth:token-type:access_token",
  audience: required("YANDEX_RELEASE_PUBLISHER_SERVICE_ACCOUNT_ID"),
